...
// Membuat query untuk mengambil data program donasi, dan menyesuaikan jika ada pencarian
if (!empty($search)) {
  // Menggunakan prepared statement untuk mencegah SQL injection
  $stmt = $conn->prepare("SELECT * FROM program_donasi WHERE judul LIKE ? OR deskripsi LIKE ?");
  $likeSearch = '%' . $search . '%';
...
<body>
  ...
  <main style="margin: 20px 50px;">
    ...
    <!-- Form Pencarian -->
    <form method="GET" action="kelola_program.php" style="margin-bottom: 20px;">
      <input type="text" name="search" placeholder="Cari program donasi..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
    ...
    <table>
      ...
      <?php else: ?>
        <tr>
          <td colspan="5" style="text-align:center;">Tidak ada program yang ditemukan.</td>
        </tr>
      <?php endif; ?>
    </table>
  </main>
